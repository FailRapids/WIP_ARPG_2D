...
export var AIR_STEERING_POWER = 100
...
func jump(delta):
	...
	var target_velocity = air_speed * _Enity.get_look_direction().normalized()
...
func _animate_jump_height(progress):
	_Enity.set_height(pow(sin(progress * PI), 0.7) * MAX_JUMP_HEIGHT)
